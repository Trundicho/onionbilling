...
public class BillingClient {
    ...
    public Invoice insert(String invoiceName) throws Exception {
        MvcResult mvcResult = this.mockMvc.perform(MockMvcRequestBuilders.post("/invoice/" + invoiceName))
                .andExpect(MockMvcResultMatchers.status().isOk()).andReturn();
        return objectMapper.readValue(mvcResult.getResponse().getContentAsString(), new TypeReference<>() {
        });
    }
    ...
    public Invoice update(Long invoiceId, Invoice invoice) throws Exception {
        ...
        MvcResult mvcResult = this.mockMvc.perform(MockMvcRequestBuilders.put("/invoice/" + invoiceId)
                .content(invoiceJson).contentType("application/json"))
                .andExpect(MockMvcResultMatchers.status().isOk()).andReturn();
        return objectMapper.readValue(mvcResult.getResponse().getContentAsString(), new TypeReference<>() {
        });
    }
    ...
    public Invoice getInvoice(Long invoiceId) throws Exception {
        MvcResult mvcResult = this.mockMvc.perform(MockMvcRequestBuilders.get("/invoice/" + invoiceId))
                .andExpect(MockMvcResultMatchers.status().isOk()).andReturn();
        return objectMapper.readValue(mvcResult.getResponse().getContentAsString(), new TypeReference<>() {
        });
    }
    ...
    public void remove(Long invoiceId) {
        try {
            this.mockMvc.perform(MockMvcRequestBuilders.delete("/invoice/" + invoiceId))
    ...
    public void getInvoiceAndExpectNotFound(long invoiceId) throws Exception {
        this.mockMvc.perform(MockMvcRequestBuilders.get("/invoice/" + invoiceId))
